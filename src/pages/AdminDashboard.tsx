...
const AdminDashboard = () => {
  ...
  const handleTransactionAction = (id: string, action: 'approve' | 'reject') => {
    const updatedTransactions = transactions.map(tx => 
      tx.id === id ? { ...tx, status: action === 'approve' ? 'approved' : 'rejected' } : tx
    );
                ...
                <TableBody>
                  {transactions.map((transaction) => (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>
                        <Badge variant={getRiskBadgeColor(transaction.riskLevel || 'low')}>
                          {transaction.riskLevel?.toUpperCase()}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <span className="font-mono">{(transaction.riskScore || 0).toFixed(2)}</span>
                      </TableCell>
                      <TableCell>
                        <span className="font-mono">{(transaction.anomalyScore || 0).toFixed(2)}</span>
                      </TableCell>
                      <TableCell>
